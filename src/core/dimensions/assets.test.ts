...
describe("assetsDimension", () => {
  it("is not applicable when there are no asset fields", () => {
    expect(assetsDimension.isApplicable?.(model({ contentTypes: [type({ id: "page", fields: [field({ id: "title" })] })] }))).toBe(false);
  });
  ...
  it("fails altText when an asset-owning type has no alt field", () => {
    ...
    const c = assetsDimension.evaluate(model({ contentTypes: [t] })).find((x) => x.id === "assets.altText");
