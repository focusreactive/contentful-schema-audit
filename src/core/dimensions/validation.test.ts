...
describe("validationDimension", () => {
  it("fails identifierUnique when a slug field lacks a unique rule", () => {
    ...
    const c = validationDimension.evaluate({ model: model({ contentTypes: [t] }) }).find((x) => x.id === "validation.identifierUnique");
  ...
  it("passes coverage when at least half of fields are validated", () => {
    ...
    const c = validationDimension.evaluate({ model: model({ contentTypes: [t] }) }).find((x) => x.id === "validation.coverage");
